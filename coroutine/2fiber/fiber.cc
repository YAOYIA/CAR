...
//当前线程上的协程控制信息
//正在运行的协程
static thread_local Fiber* t_fiber = nullpter;
//主协程
static thread_local std::shared_ptr<Fiber> t_thread_fiber = nullpter;
//调度协程
...
Fiber::Fiber(std::function<void()> cb,size_t stacksize,bool run_in_schduler):m_cb(cb),m_runInScheduler(run_in_schduler)
{
    m_state = READY;
    //分配协程栈空间
